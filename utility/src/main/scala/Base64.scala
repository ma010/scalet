...
object Base64 extends App {
  ...
  val strDecoded = org.apache.commons.codec.binary.Base64.decodeBase64(base64Str)

  print(strDecoded.mkString)
  ...
  def encodeString(in: Array[Byte]): String = new String(Base64.encode(in))

  /** Encodes the given String into a Base64 ByteArray. **/
  def encodeBinary(in: String): Array[Byte] = Base64.encode(in.getBytes("UTF-8"))

  /** Encodes the given ByteArray into a Base64 ByteArray. **/
  def encodeBinary(in: Array[Byte]): Array[Byte] = Base64.encode(in)
